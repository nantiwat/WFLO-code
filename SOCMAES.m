function SOCMAES1(fun,foutput,nloop,nsol,nvar,nbit,a,b)
...
% cmaes.m, Version 2.42, last change: September, 16, 2006 CMAES
% implements an Evolution Strategy with Covariance Matrix Adaptation
% (CMA-ES) for nonlinear function minimization.  For introductory
% comments and copyright see end of file (type 'type cmaes'). It
% should run with MATLAB (Windows, Linux) and Octave (Linux, package
% octave-forge is needed).
%
...
% run is stopped if the first two non-white sequences in any line of
% this file are 'stop' and the value of the SaveFileName option (by
% default 'variablescmaes.mat').
% 
...
opts.lbounds=a(1);
opts.ubounds=b(1);
opts.MaxFunEvals=nloop*nsol;
opts.Plotting='off';
sigm0=sqrt(var(x0')');
[xmin,fpmin,fcout,stflag,historyfile]=cmaes(fun,x0,sigm0,opts);
[fpmin,fmin,gmin]=feval(fun,xmin);
save(foutput,'xmin','fpmin','fmin','gmin','historyfile')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
defopts.EvalInitialX = 'yes  % evaluation initial solution';
...
% TODO: keep also median solution? 
  ...
  out.hist.param.comment = ['maxD=sqrt(max(EV)) where EV are the eigenvalues' ...
      ' of C and sigma^2*C is the covariance matrix of the search distribution'];
  ...
  out.histParamArr.comment = ['diagD = sort(sqrt(EV)), Bmax = eigenvector of largest ' ...
		    ' eigenvalue (EV) of C'];
...
while isempty(stopflag)
  ...
  counteval = counteval + lambda; % retries due to NaN are not counted
...
%               num2str([countiter counteval]) ']']);
  ...
  if countiter == 1
	...
	out.solutions.bestever.x = xold;
	out.solutions.bestever.f = fitness.hist(2);
	out.solutions.bestever.evals = 1;
    else
  ...
  elseif fitness.hist(1) < out.solutions.bestever.f
    out.solutions.bestever.x = arxvalid(:, fitness.idx(1));
    out.solutions.bestever.f = fitness.hist(1);
    out.solutions.bestever.evals = counteval + fitness.idx(1) - lambda;
    bestever = out.solutions.bestever;
...
% Changes: 
% 09/16: output argument bestever inserted again for convenience and
