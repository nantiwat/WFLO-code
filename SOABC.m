function SOABC(fun,foutput,nloop,nsol,nvar,nbit,a,b)
...
if floor(nsol/2)==nsol/2
    NP=nsol; %/* The number of colony size (employed bees+onlooker bees)*/
else
    NP=nsol+1;
end
FoodNumber=NP/2; %/*The number of food sources equals the half of the colony size*/
limit=100; %/*A food source which could not be improved through "limit" trials is abandoned by its employed bee*/
maxCycle=nloop; %/*The number of cycles for foraging {a stopping criteria}*/


%/* Problem specific variables*/
objfun=fun; %cost function to be optimized
D=nvar; %/*The number of parameters of the problem to be optimized*/
ub=b'; %/*lower bounds of the parameters. */
lb=a';%/*upper bound of the parameters.*/
...
%%%%%%%%% EMPLOYED BEE PHASE %%%%%%%%%%%%%%%%%%%%%%%%
       ...
       %  /*if generated parameter value is out of boundaries, it is shifted onto the boundaries*/
            ...
            % apply chaose first 
            [ObjValSol,fobj,gconst]=feval(objfun,sol');
...
%In Basic ABC, only one scout is allowed to occur in each cycle*/
        ...
        if (trial(ind)>limit)
            ...
            FVal(ind)=fobj;GVal(:,ind)=gconst;
        end;


        [fpmin,npmin]=min(ObjVal);
        fpminhist=[fpminhist fpmin];fpavghist=[fpavghist mean(ObjVal)];
        fpmaxhist=[fpmaxhist max(ObjVal)];
        fhist=[fhist FVal(npmin)];ghist=[ghist GVal(:,npmin)];
...
xmin=GlobalParams';[fpmin,fmin,gmin]=feval(fun,xmin);
maxeval=nloop*nsol;
